...
// function to store individual  heroes in local storage
const spiderMan = heroArray[0];
const ironMan = heroArray[1];
const hulk = heroArray[2];
const thor = heroArray[3];
const cptAmerica = heroArray[4];

function storeHero(indHeroArray) {

    localStorage.setItem('spiderMan', JSON.stringify(IndHeroArray));
    
}

function getStoredIndHero() {

    let indHeroArray = [];
    
    if (localStorage.getItem('spiderMan') != null) {
        indHeroArray = JSON.parse(localStorage.getItem('spiderMan'));
        return indHeroArray;
    } else {
        return indHeroArray;
    }
